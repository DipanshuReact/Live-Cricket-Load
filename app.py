...
def show_load():
    ...
    data = json.loads(response.text)  # parse JSON list
    ...
    for match in data:
        ...
        if total_match_bet > 0:
            match["team1_match_pct"] = round(team1_bet / total_match_bet * 100, 2)
            match["team2_match_pct"] = round(team2_bet / total_match_bet * 100, 2)
        else:
            match["team1_match_pct"] = match["team2_match_pct"] = 0
        ...
        if total_toss_bet > 0:
            match["team1_toss_pct"] = round(toss1_bet / total_toss_bet * 100, 2)
            match["team2_toss_pct"] = round(toss2_bet / total_toss_bet * 100, 2)
        else:
            match["team1_toss_pct"] = match["team2_toss_pct"] = 0
